/* mainMentor.js — Lógica comum a todas as telas do mentor
   Pulsar · Pulse Mais
   Responsabilidades:
     - Auth guard: redireciona para login se não houver token ou perfil != Mentor
...
(function authGuard() {
  ...
  if (!payload || payload.perfil !== 'Mentor') {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* Preenche saudação na welcome card (só existe no dashboard) */
  const welcomeNome = document.getElementById('welcomeNome');
  if (welcomeNome && window.currentUser?.nome) {
    welcomeNome.textContent = `Olá, ${window.currentUser.nome.split(' ')[0]}`;
  }

  /* Foto de perfil do mentor no topbar (fonte: /me → usuarios.foto_url) */
