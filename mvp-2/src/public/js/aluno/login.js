/* login.js — Login + Cadastro multi-etapas
   Pulsar · Pulse Mais
*/
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (form) {
    form.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        window.api.setUser(data.usuario);

        const rotas = {
          Aluno:       '/pages/aluno/dashboard.html',
          Mentor:      '/pages/mentor/dashboardMentor.html',
          GestaoGeral: '/pages/gestão/dashboardGestao.html',
          Coordenacao: '/pages/gestão/dashboardGestao.html',
          Assistente:  '/pages/gestão/dashboardGestao.html',
        };
  ...
  document.getElementById('reg-btn-submit')?.addEventListener('click', async () => {
    ...
    try {
      ...
      // Oculta stepper, steps, nav e header
      ...
      const tick = setInterval(() => {
        ...
        if (segundos <= 0) {
          ...
          // Restaura elementos do painel de cadastro
          ...
          // Limpa campos e volta para etapa 1
          limparFormCadastro();
          irParaEtapa(1);
